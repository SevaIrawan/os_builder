...
| 11 | ✅ COMPLETED 2026-09-22 · dialihkan ke 页首附表 N03 身份件 (`277d67ba2b04`) · mendarat di **v38** (07:02:32Z) · bukti baca-balik: `diffConfluenceContentVersions` v37↔v38 **additions 1 / deletions 1**, kalimat lama utuh. Sasaran lama (baris 协作 Thread di 区六) memang tidak ada dan tidak perlu ada — 区六 memuat komponen yang **dipanggil** S-05 |
...
| 34 | ✅ COMPLETED 2026-09-22 · mendarat di **v40** (07:46:03Z) · bukti baca-balik: badan v40 dibaca ulang dari server, uji balik **sama persis karakter demi karakter** dengan v39 setelah 9 nilai baru dikembalikan ke teks asalnya; 2.185 `data-local-id` utuh (nol hilang nol ditambah), nol tag menggantung, nol tabel bergerigi — 区三 字段清单 (`d60d1c617fdf`): 13 cf ID hasil **回读 API sendiri** lewat `createmeta` — cf18243/18252/18244/18256, enam field PIP (18248/18249/18250/18253/18245/18251), 18254, 18255, 18257 — lengkap dengan tipe dan nilai option; tiga selisih redaksi dengan c50345 dicatat apa adanya (prefiks nama PIP, cf18257 juga ada di Sub-ticket 14316, cf18251 ternyata textfield); **batas verifikasi ditulis terang**: hanya HR/BO/FIN yang terbukti, CRM/FOZ/WP/XLP tidak bisa dibuktikan karena akun ini tak punya hak buat issue di sana — sudah dipasangi probe pembanding 07.06.1 E16 (`getVisibleJiraProjects(action=create)` mengembalikan 8 project, keempatnya tidak termasuk) |
| 35 | ✅ COMPLETED 2026-09-22 · mendarat di **v40** (07:46:03Z) · bukti baca-balik: badan v40 dibaca ulang dari server, uji balik **sama persis karakter demi karakter** dengan v39 setelah 9 nilai baru dikembalikan ke teks asalnya; 2.185 `data-local-id` utuh (nol hilang nol ditambah), nol tag menggantung, nol tabel bergerigi — paragraf butir 14／31 (`e9cd09bd56c2`): OSD-116 **c50345** Kent 14:24 dikutip apa adanya 「built and registered in 04.10 §三 (v20). The field side is ready — you are no longer blocked on it.」; ketergantungan sisi Schema Owner dinyatakan **selesai**, sementara c50279 (Alden, 模式九) dinyatakan **masih menggantung** — c50345 sendiri menulis 「is separate」 |
| 36 | ✅ COMPLETED 2026-09-22 · mendarat di **v40** (07:46:03Z) · bukti baca-balik: badan v40 dibaca ulang dari server, uji balik **sama persis karakter demi karakter** dengan v39 setelah 9 nilai baru dikembalikan ke teks asalnya; 2.185 `data-local-id` utuh (nol hilang nol ditambah), nol tag menggantung, nol tabel bergerigi — daftar versi sumber (`613d46be24bf`): 04.10 **v19 → v20** (2026-09-22 07:22:35Z, baris registrasi 13 field ditulis Kent). Koreksi sebelumnya yang menyebut 「四处过期」 dinyatakan harus dibaca 「五处」 |
| 37 | ✅ COMPLETED 2026-09-22 · mendarat di **v40** (07:46:03Z) · bukti baca-balik: badan v40 dibaca ulang dari server, uji balik **sama persis karakter demi karakter** dengan v39 setelah 9 nilai baru dikembalikan ke teks asalnya; 2.185 `data-local-id` utuh (nol hilang nol ditambah), nol tag menggantung, nol tabel bergerigi — 区一 enam baris 状态 — N10 (`649d24ea-…`), N12 (`ac9ad7db-…`), N14 (`75abe7b2-…`), N16 (`5edab308-…`), N17 (`e9c08849-…`), N25 (`6e853153-…`): masing-masing ditempeli catatan bahwa sisi field **bukan lagi prasyarat** baris itu; teks 未建（可建） dan prasyarat lain di baris tersebut **tidak diubah** |
