...
| | |
|---|---|
| **Versi halaman 建造单 sekarang** | **v35** (2026-09-22 06:37:03.616Z) — ditulis lewat akun Bambang, jalur `edits` node-level |
| **⏳ PENDING** (menunggu, belum ditulis ke Confluence) | **2** — butir 11 dan **butir 21 (baru)** |
| **✅ COMPLETED** (sudah ditulis ke Confluence) | **18** |
| **👁 PANTAU** (tidak mengubah baris apa pun, cuma diawasi) | **1** — butir 16 |
| **Terakhir ditulis ke Confluence** | **2026-09-22 06:37:03Z — v34 → v35** (butir 15, satu `insertNodeAfter`, diff additions 1 / deletions 0). Sebelumnya **06:27:52Z — v33 → v34**, 15 operasi `edits` (11 `replaceNode` ＋ 4 `insertNodeBefore`). Didahului `dryRun` yang lulus uji balik: hasil dikurangi sisipan sama persis dengan v33, 1.964 `data-local-id` lama utuh, nol hilang |
...
| 21 | ⏳ PENDING | 页首附表 — paragraf 统计 tepat di bawah tabel (`data-local-id="a99c1693d3d4"`) | Paragrafnya berbunyi 「共 33 行——阻塞中 5…；待办 24…；已解封 4」 (5＋24＋4＝33). Butir 15 mendarat di **v35** dan menambah satu baris 待办, jadi tabel sekarang **34 baris / 待办 25**. Angka di paragraf itu **sudah tidak akurat sejak 2026-09-22 06:37:03Z**. Sudah dicatat di pesan versi v35 supaya tidak tersembunyi. **Belum diperintahkan Bambang, jadi belum ditulis.** | 「【2026-09-22 订正｜原文保留不删】上句「共 33 行……待办 24」为 v34 及以前的计数。v35（2026-09-22）依建造人指示新增「S-05 各 n8n 件切 active 的次序」一行（待办，依赖 Kayden），本表现为 **34 行——阻塞中 5；待办 25；已解封 4**，其余分项口径不变。」 | Diff sisi server v34↔v35 (additions 1 / deletions 0); hitungan `<tr>` dalam tabel `952c32b3be2f`: 33 → 34 |
