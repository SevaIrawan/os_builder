...
| 11 | ⏳ PENDING — **dialihkan sasarannya, TIDAK digugurkan** (ditelusuri 2026-09-22). Sasaran lama (baris 协作 Thread di 区六) memang tidak ada — 区六 punya tepat 7 baris dan judul kolomnya 「本流程用它做什么」, sedangkan S-05 tidak memanggil komponen itu. **Tetapi isinya tetap perlu**: 页首附表 baris N03 身份件 (`277d67ba2b04`) masih menulis 「04.4 §十一 **v31** 现行值…身份件与协作 Thread 两件为「在建」」, sedangkan 区六 身份件 (koreksi butir 10, sudah mendarat v34) menulis 协作 Thread 「可用」 — **dua nilai berbeda di halaman yang sama**. Dibaca langsung dari 04.4 **v33** §十一: 协作 Thread ＝ 可用 (c50273; 仍 inactive、零真跑), 身份件 ＝ 在建 → v33: 可用 6 件, 在建 1 件. Usul: satu `replaceNode` pada `277d67ba2b04`, 原文保留不删. Lihat `audit-buildsheet-v36-2026-09-22.md` §7 |
