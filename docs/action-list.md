...
| A-06 | Bangun 守护件 N28 (方向 2): konfigurasi izin transisi `Abort Case` (id 11) ke grup peran **HR Ops & Data**, plus komponen penjaga yang DM ke HR Ops & Data saat case masih berjalan tapi profil karyawan sudah berubah jadi *departing/departed* | Kent OSD-116 **c50255** (2026-09-21), mengeksekusi putusan Kayden c50244 | Kent menulis: 「N28 case-void → Direction 2 for now (don't touch the frozen Spec)… Direction 1 (auto-void) = post-launch v2」. Ini **menutup B-01 butir (b) dan (c)** — dua-duanya memang diserahkan ke kita. Tidak menunggu siapa pun |
...
**Yang pindah keluar dari bagian ini hari ini**: A-01 → D-12 · A-02 → D-13 · A-05 → D-14.
...
| B-08 | **Alden** | Buku detail 04.9 untuk S-05 | 2026-09-20 | Aturan besi 04.9 §一 "tiga serangkai". **2026-09-21**: 04.9 naik v103 → **v106**. Yang ditambahkan: baris indeks 身份件 dan pembaruan baris keluarga Grade. **Tidak ada buku baru untuk S-05** — dibaca dari diff v103→v106 |
...
| B-15 | **Kayden** | Apakah N28 perlu pengecualian terhadap 04.3 §六 互斥条 | 2026-09-21 | Dirutekan oleh **c50263** kita. 判据 baris blocker 建造单 no.19 minta dua hal: 执行身份 (sudah beres lewat c50244 → c50257 方向 2) dan 互斥条适用口径 (ini, belum). 04.3 dicek hari ini: masih v33, §六 tidak berubah |
...
| C-08 | Dua file daftar di repo ini — `docs/action-list.md` (file ini) dan `docs/pending-buildsheet-updates.md` — mau dipisah terus atau digabung | pisah / gabung | Aturan di kepala file ini berbunyi 「**Satu-satunya file daftar aksi.** Kalau ada aksi baru, masuk ke sini — jangan bikin file lain」. `pending-buildsheet-updates.md` bukan daftar aksi (isinya teks yang akan ditulis ke Confluence), dan `buildsheet-blocker-status.md` juga bukan (cermin tabel 附表). Tapi ketiganya memang bersinggungan. **Aku tidak menggabungkan apa pun tanpa perintah kau** |
